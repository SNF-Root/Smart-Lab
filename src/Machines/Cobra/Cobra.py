from src.Machines.Cobra.Job import Job
...
class Cobra(Runner_Base):
    """
    Iterates through all Oxford Instruments PlasmaPro 100 Cobra ICP etchers (PTIQ control
    software) registered in register.txt and runs the Job algorithm for each of them. Each
    physical tool (e.g. Ox-ALE, Ox-Gen) is registered as its own named Cobra instance.

    Unlike the other machines in this project, a Cobra's process history lives in the PTIQ
    ...
    TODO: PTIQ also has a streamed accumulation of the process data, should we account/parse this too?
    ...
    run()
        Runs the Job algorithm for all Cobra machines and uploads the results to the cloud storage.
    """

    def __init__(self):
        """
        Constructor for the Cobra class.
